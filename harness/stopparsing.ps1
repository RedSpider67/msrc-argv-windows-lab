...
$work = Join-Path $env:RUNNER_TEMP ("sp-" + [guid]::NewGuid().ToString('N').Substring(0,8))
...
Leg "CONTROL  undefined percent variable"          "$base\Fab%MSRC_UNDEFINED_VAR%rikam.dacpac"
...
Leg "INJECT   defined percent variable"            "$base\Fab%MSRC_PLAIN%rikam.dacpac"
Leg "INJECT   percent variable carrying spaces"    "$base\Fab%MSRC_SPLIT%rikam.dacpac"
Leg "INJECT   percent variable carrying a quote"   "$base\Fab%MSRC_QUOTE%rikam.dacpac"
Leg "INJECT   percent variable carrying percents"  "$base\Fab%MSRC_NEST%rikam.dacpac"
...
Write-Output ""
Write-Output "--- CHECKED-OUT FILE NAME SURVIVAL (git + NTFS on this runner)"
Get-ChildItem (Join-Path $here '..\repo-payloads') -ErrorAction SilentlyContinue | ForEach-Object { Write-Output ("    " + $_.Name) }
Write-Output ""
Write-Output "DONE"
