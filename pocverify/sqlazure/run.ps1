...
# Run: powershell -NoProfile -File run.ps1   (or pwsh -NoProfile -File run.ps1)

$ErrorActionPreference = 'Continue'
$lab = Join-Path ([System.IO.Path]::GetTempPath()) ("sqlazlab-" + [guid]::NewGuid().ToString('N').Substring(0,8))
Write-Host ("PowerShell " + $PSVersionTable.PSVersion + " (" + $PSVersionTable.PSEdition + ") on " + [System.Environment]::OSVersion.Platform)
Write-Host ""
